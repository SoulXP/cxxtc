...
consteval int size_of_cxxtctimecode()
{
#if defined(_WIN32)
    constexpr int size = (sizeof(cxxtc::timecode::scalar_t) * TOTAL_SCALAR_GROUPS) +
                          sizeof(cxxtc::timecode::fps_scalar_t) +
                          sizeof(cxxtc::timecode::flags_t) +
                          sizeof(void*);
    ...
    constexpr int padding = to_nearest_padding(size, alignof(cxxtc::timecode));

    return size + padding;
}

// conditions for static tests
static_assert(sizeof(cxxtc::timecode) == size_of_cxxtctimecode(), "size of cxxtc::timecode object does not meet established size requirements");
...
TEST_CASE("cxxtc::timecode initialization", "[timecode][chrono][initialization]")
{
    SECTION("default intialization") {
        ...
        INFO("tc1 fps: "       << tc1.fps());       CHECK(tc1.fps()       == cxxtc::fps::default_value());
    ...
    SECTION("copy construction") {
        ...
        INFO("tc1 fps: "       << tc1.fps());       REQUIRE(tc1.fps()       == cxxtc::fps::fps_24);

        // destination object
        cxxtc::timecode tc2{ tc1 };
        ...
        INFO("tc2 fps: "       << tc2.fps());       CHECK(tc2.fps()       == cxxtc::fps::fps_24);
        ...
        INFO("tc1 fps: "       << tc1.fps());       CHECK(tc1.fps()       == cxxtc::fps::fps_24);
    ...
    SECTION("move construction") {
        ...
        // destination object
        cxxtc::timecode tc2{ std::move(tc1) };
    ...
    SECTION("integral subsumption initialization"){
        // source object
        cxxtc::timecode tc1{ (1 * 60 * 60 * 25 * 100) +
                           (2 * 60 * 25 * 100) +
                           (3 * 25 * 100) +
                           (4 * 100) +
                            5
        ...
        // pre-conditions for valid copy construction
        INFO("tc1 fps: "       << tc1.fps());       REQUIRE(tc1.fps()       == cxxtc::fps::default_value());
    ...
    SECTION("string view subsumption initialization with extended timecode form"){
        // source object
        cxxtc::timecode tc1{ "00:00:00:00.00" };
        cxxtc::timecode tc2{ "12:34:56:12.34" };

        // pre-conditions for valid copy construction
        INFO("tc1 fps: "       << tc1.fps());       REQUIRE(tc1.fps()       == cxxtc::fps::default_value());
        INFO("tc2 fps: "       << tc2.fps());       REQUIRE(tc2.fps()       == cxxtc::fps::default_value());
    ...
    SECTION("string view subsumption initialization with standard timecode form"){
        // source object
        cxxtc::timecode tc1{ "00:00:00:00" };
        cxxtc::timecode tc2{ "12:34:56:12" };
...
TEST_CASE("cxxtc::timecode value modification semantics", "[chrono][timecode][conversion][number]")
{
    SECTION("groups can be set independently") {
        ...
        INFO("tc1 fps: "       << tc1.fps());       REQUIRE(tc1.fps()       == cxxtc::fps::fps_25);
    }
...
TEST_CASE("cxxtc::timecode tick value representation", "[chrono][timecode][number]")
{
...
TEST_CASE("cxxtc::timecode integral assignment", "[chrono][timecode][conversion][number]")
{
    SECTION("assignment to signed types return correct tick values") {
        ...
        INFO("tc1 fps: "    << tc1.fps()); REQUIRE(tc1.fps() == cxxtc::fps::default_value());
        INFO("tc2 fps: "    << tc2.fps()); REQUIRE(tc2.fps() == cxxtc::fps::default_value());
        INFO("tc3 fps: "    << tc3.fps()); REQUIRE(tc3.fps() == cxxtc::fps::default_value());
        INFO("tc4 fps: "    << tc4.fps()); REQUIRE(tc4.fps() == cxxtc::fps::default_value());
...
TEST_CASE("cxxtc::timecode signed type conversions", "[chrono][timecode][conversion][number]")
{
    SECTION("signed type conversion returns correct ticks for each TC group") {
        ...
        // destination object
        cxxtc::timecode::signed_type _int1 = 0;
...
TEST_CASE("cxxtc::timecode string conversions", "[chrono][timecode][conversion][string]")
{
...
TEST_CASE("cxxtc::timecode equality & comparison", "[timecode][chrono][comparison][equality][ordering]")
{
    SECTION("equivalent timecode objects with same fps returns true") {
        // default objects
        cxxtc::timecode tc1{"12:34:56:12.34"};
        cxxtc::timecode tc2{"12:34:56:12.34"};
    ...
    SECTION("inequivalent timecode objects with same fps returns false") {
        // default objects
        cxxtc::timecode tc1{"12:34:56:12.34"};
        cxxtc::timecode tc2{"34:12:56:12.34"};
    ...
    SECTION("equivalent timecode objects with different fps returns false") {
        ...
        cxxtc::timecode tc1{"12:34:56:12.34"};
        cxxtc::timecode tc2{"12:34:56:12.34"};
        tc2.set_fps(cxxtc::fps::fps_30);

        // pre-conditions for test
        INFO("tc1 fps: "    << tc1.fps()); REQUIRE(tc1.fps() == cxxtc::fps::default_value());
        INFO("tc2 fps: "    << tc2.fps()); REQUIRE(tc2.fps() == cxxtc::fps::fps_30);
    ...
    SECTION("timecode objects with same fps returns correct ordering") {
        ...
        cxxtc::timecode tc1{"12:34:56:12.34"};
        cxxtc::timecode tc2{"12:34:56:12.34"};
        cxxtc::timecode tc3{"00:34:56:12.34"};
        cxxtc::timecode tc4{"34:34:56:12.34"};
...
TEST_CASE("cxxtc::timecode arithmetic operations", "[timecode][chrono][arithmetic]")
{
    SECTION("addition of timecode objects with same fps yields correct results") {
        // default objects
        cxxtc::timecode tc1{"00:00:00:00.00"};
        cxxtc::timecode tc2{"00:00:00:00.42"};
        cxxtc::timecode tc3{"00:00:00:12.00"};
        cxxtc::timecode tc4{"00:00:42:00.00"};
        cxxtc::timecode tc5{"00:42:00:00.00"};
        cxxtc::timecode tc6{"42:00:00:00.00"};
        ...
        INFO("tc3 fps: "      << tc3.fps()); REQUIRE(tc3.fps() == cxxtc::fps::default_value());
        INFO("tc4 fps: "      << tc4.fps()); REQUIRE(tc4.fps() == cxxtc::fps::default_value());
        INFO("tc5 fps: "      << tc5.fps()); REQUIRE(tc5.fps() == cxxtc::fps::default_value());
        INFO("tc6 fps: "      << tc6.fps()); REQUIRE(tc6.fps() == cxxtc::fps::default_value());
    ...
    SECTION("subtraction of timecode objects with same fps yields correct results") {
        // default objects
        cxxtc::timecode tc1{"42:42:42:12.42"};
        cxxtc::timecode tc2{"00:00:00:00.42"};
        cxxtc::timecode tc3{"00:00:00:12.00"};
        cxxtc::timecode tc4{"00:00:42:00.00"};
        cxxtc::timecode tc5{"00:42:00:00.00"};
    ...
    SECTION("multiplication of timecode objects with same fps yields correct results") {
        // default objects
        cxxtc::timecode tc1{"00:00:00:00.01"};
        cxxtc::timecode tc2{"00:00:00:00.42"};
        cxxtc::timecode tc3{"00:00:00:12.00"};
        cxxtc::timecode tc4{"00:00:42:00.00"};
        cxxtc::timecode tc5{"00:42:00:00.00"};
        ...
        // conditions for test
        INFO("tc1 * tc2: " << (tc1 * cxxtc::timecode{0}).ticks()); CHECK(tc1 * cxxtc::timecode{0} == 0);
        INFO("tc1 * tc2: " << (tc1 * tc2).ticks()); CHECK(tc1 * tc2 == 42);
        INFO("tc1 * tc3: " << (tc1 * tc3).ticks()); CHECK(tc1 * tc3 == (12 * 100));
        INFO("tc1 * tc4: " << (tc1 * tc4).ticks()); CHECK(tc1 * tc4 == (42 * 25 * 100));
        INFO("tc1 * tc5: " << (tc1 * tc5).ticks()); CHECK(tc1 * tc5 == (42 * 60 * 25 * 100));
    ...
    SECTION("division of timecode objects with same fps yields correct results") {
        // default objects
        cxxtc::timecode tc1{"00:00:00:00.02"};
        cxxtc::timecode tc2{"00:00:00:00.42"};
        cxxtc::timecode tc3{"00:00:00:12.00"};
        cxxtc::timecode tc4{"00:00:42:00.00"};
        cxxtc::timecode tc5{"00:42:00:00.00"};
